...
def cal_target_center(attacker_clean_vec_dict, attacker_selected, attacker_list, kernel_bandwidth):
    attacker_center_dict = {}
    for attacker in attacker_list:
        target_clean_vecs = attacker_clean_vec_dict[attacker]
        dim = attacker_selected[attacker]
        data = target_clean_vecs[dim]
        points, shift_points, cluster = train_mean_shift(data, kernel_bandwidth)
        center = np.array(shift_points[0])
        attacker_center_dict[attacker] = center
    return attacker_center_dict
