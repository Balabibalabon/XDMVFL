...
def search_vec(attacker_center_dict, attacker_clean_vec_dict, args, attacker_list):
    attacker_target_vec_dict = {}
    for attacker in attacker_list:
        center = torch.tensor(attacker_center_dict[attacker])

        target_clean_vecs = torch.tensor(attacker_clean_vec_dict[attacker])
        ...
        target_vec.reshape(1,args.bottomModel_output_dim)
        attacker_target_vec_dict[attacker] = target_vec
